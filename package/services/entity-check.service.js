...
let EntityCheckService = class EntityCheckService {
    constructor(entityManager) {
        this.entityManager = entityManager;
    }
    async checkNameExist(entityClass, name) {
        const exist = await this.entityManager.findOne(entityClass, { name });
        if (exist) {
            throw new common_1.HttpException(i18n_1.__('Name already exists'), 409);
        }
    }
};
EntityCheckService = __decorate([
    common_1.Injectable(),
